...
def show_evaluation_form():
    ...
    if rubric_type == "cs_programming":
        ...
        **Competency Areas (8 items total):**
        ...
        """)
    elif rubric_type == "ster":  # STER evaluation
        ...
        **Competency Areas (19 items total):**
        ...
        st.info("📋 **Custom Rubric** - Upload JSON rubric or use sample CS rubric")
        with st.expander("Rubric Source", expanded=True):
            col1, col2 = st.columns(2)
            with col1:
                uploaded_rubric = st.file_uploader("Upload Rubric JSON", type=["json"], key="custom_rubric_uploader")
            with col2:
                use_sample = st.checkbox("Use sample CS rubric", value=True)
        ...
        if custom_rubric is None:
            st.warning("Please upload a rubric JSON or use the sample rubric to proceed.")
            return

        items = rubric_to_items(custom_rubric)
        st.caption(f"Loaded {len(items)} criteria from rubric: {custom_rubric.get('name')}")
        # Store rubric metadata for persistence/export
        st.session_state.rubric_meta = {
            'name': custom_rubric.get('name', 'Custom Rubric'),
            'version': custom_rubric.get('version', '1.0'),
            'scale': custom_rubric.get('scale', {'min': 0, 'max': 3}),
            'criteria_count': len(custom_rubric.get('criteria', []))
        }
                            ...
                            try:
                                ...
                                if rubric_type == "cs_programming" and custom_rubric is not None:
                                    ...
                                    result = openai_service.generate_feedback_for_custom_rubric(
                                        ...
                                        context=lesson_plan_context,
                                        student_identifier=student_name
                                    )
                                    # Map to analyses/justifications using rubric item ids
                                    ai_analyses = {}
                                    feedback_map = result.get('feedback', {})
                                    for item in items:
                                        ai_analyses[item['id']] = feedback_map.get(item['id'], '')
                                else:
                                    # Generate AI analysis for preset items (field/ster)
